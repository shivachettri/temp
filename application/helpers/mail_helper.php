<?php
function send_mail($email, $subject, $message, $person_name, $button_name = false, $button_link = false)
{
	include_once "Mailer/class.phpmailer.php";

	$mail = new PHPMailer();
	$mail->IsSMTP();
	$mail->SMTPDebug = 0; // 0: No Debugging / 1 = Error & Message / 2 = Message Only
	$mail->SMTPAuth = true;
	$mail->SMTPSecure = 'ssl';
	$mail->Host = "smtp.gmail.com";
	$mail->Port = 465;
	$mail->IsHTML(true);
	$mail->Username = "bookmyspace.2021@gmail.com";
	$mail->Password = "\$BookMySpace#3";
	$mail->SetFrom("bookmyspace.2021@gmail.com");
	$mail->Subject = $subject;
	$mail->Body = get_template($person_name, $message, $button_name, $button_link);
...
function get_template($person_name, $message, $button_name = false, $button_link = false)
{
	$button = "";
	if ($button_name != false && $button_link != false) {
		$button = '
						...
						<td align="left">
							<table role="presentation" border="0" cellpadding="0" cellspacing="0">
								<tbody>
									<tr>
										<td> 
											<a href="' . $button_link . '" target="_blank">' . $button_name . '</a> 
										</td>
									</tr>
								</tbody>
							</table>
	...
	return '
		...
		<body class="">
			<span class="preheader">This email is from ' . PROJECT_NAME . '!</span>
								...
								<tr>
									<td class="wrapper">
										<table role="presentation" border="0" cellpadding="0" cellspacing="0">
											<tr>
												<td>
													<h1 style="font-weight: 500;text-align:left;">' . PROJECT_NAME . '</h1>
													<br/>
													<h2>Greetings ' . $person_name . ',</h2>
													<h4>' . $message . '</h4>
													' . $button . '
													<br/>
													<p>Please write to us for all queries at ' . PROJECT_EMAIL . '</p>
												</td>
											</tr>
										</table>
									</td>
								...
								<table role="presentation" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td class="content-block">
											<span class="apple-link">' . PROJECT_ADDRESS . '</span>
											<br> 
											<span class="apple-link">' . PROJECT_EMAIL . '</span>
											&nbsp;&nbsp; / &nbsp;&nbsp;
											<span class="apple-link">' . PROJECT_PHONE . '</span>
											<br/>
											<span>www.csr2life.com</span>
										</td>
									</tr>
								</table>
							</div>
